...
void HttpRequest::setVersion(const Version &version) { version_ = version; }
...
const string &HttpRequest::getPath() const { return path_; }
...
const string &HttpRequest::getQuery() const { return query_; }

void HttpRequest::setReceivedTime(const std::string &t) { receivedTime_ = t; }

const string &HttpRequest::getReceivedTime() const { return receivedTime_; }
                            ...
                            const char *end)
{
  ...
  // TODO 下面一行的作用？
  for (; !value.empty() && isspace(value[value.size() - 1]);
...
const map<string, string> &HttpRequest::getHeaders() const { return headers_; }
