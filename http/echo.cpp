...
void Echo::messageCallback(TcpConnect::ptr tcpConnect, Buffer &Buffer)
{
...
void Echo::writeCompleteCallback(yy::TcpConnect::ptr tcpConnect)
{
  cout << "thread id:" << std::this_thread::get_id() << endl;
  string addr = tcpConnect->getAddr().toString();
  cout << addr << ":"
...
void Echo::connectCloseCallback(yy::TcpConnect::ptr tcpConnect)
{
  string addr = tcpConnect->getAddr().toString();
